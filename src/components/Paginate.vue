...
<template>
    <div class="flex items-center gap-2">
        ...
        <button role="button" v-if="current != 1 && current != total" @click="handleChangePage(current)"
            class="size-8 text-dark bg-very_grey rounded-md cursor-pointer">
            {{ current }}
        </button>
        ...
        <button role="button" @click="handleChangePage(total)" :class="`${current == total ? 'text-dark bg-very_grey' : 'text-very_grey border border-very_grey'}
